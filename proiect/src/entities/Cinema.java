...
public class Cinema {
        ...
        for(Movie m: room_numbers.keySet()){
            if(room_numbers.get(m) < 0 || room_numbers.get(m) > this.nr_rooms)
                ...
                if (!found)
                    ...
                    this.room_numbers.put(m, room_numbers.get(m));
            }
        }
    }
    ...
    public void setSeats_available(HashMap<Movie, Integer> seats_available) {
        for(Movie m: seats_available.keySet()){
            if(seats_available.get(m) < 0 || seats_available.get(m) > this.nr_seats[this.room_numbers.get(m)-1])
                ...
                if (!found)
                    ...
                    this.seats_available.put(m, seats_available.get(m));
            }
        }
    }
